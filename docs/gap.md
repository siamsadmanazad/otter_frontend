...
| **G5** | **Settings tabs** (notifications/privacy/business) | **DONE (code)** — hosted migration apply pending DB password | P1 | M | FE+BE | A1 migration (`profiles.preferences`) **validated locally**; API (GET defaults / PATCH deep-merge) + 3 forms + enforced `whoCanMessage` on chat-create (degrades gracefully pre-migration). **Last step:** `supabase db push` to hosted (needs DB password). |
| **G6** | **Chat (realtime DM)** send/receive | **DONE (code) 2026-06-20** — needs 2-user browser verify | P1 | L | FE | W6a backend + W6b UI both done. Direct DMs on REST + one RLS-scoped Realtime channel; new-chat picker (people you follow); unread dot; load-older. `/chat` serves 200 in dev. **Fast-follow:** per-message "seen" receipts; group chat. |
...
| **G9** | **Seed data** (demo users/posts/tribes) | **DONE 2026-06-20 — applied to hosted** | P1 | S | BE | `scripts/seed.mjs` (npm `seed`/`seed:purge`): 8 users, follows, 10 posts (2 journals), 3 tribes, a chat, notifications. Idempotent (verified). Live on hosted. Demo pw `OtterDemo!2026`. |
| **G10** | **Deploy** web to Vercel (env, build, domain) | Not deployed | P0 | M | FE | Soft launch needs a live URL. |
| **G11** | **Shop/product** uses mock data; `product/[id]` parses int ids | **DONE (hidden) 2026-06-20** | P2 | M | FE | `SHOP_ENABLED=false` flag: nav entries hidden, /shop + /shop/[id] + /product/[id] redirect home. Code kept for fast-follow (uuid-back later). |
